// Copyright (c) 2017 CORAL-CMU

#include "ntrip.h"

#define NTRIPWAIT 30000

Ntrip::Ntrip() : QTcpSocket(Q_NULLPTR) {
  receiveflag = false;
  thread = new QThread();
  this->moveToThread(thread);
  connect(this, SIGNAL(signalLoadCasterList()), this,
          SLOT(slotLoadCasterList()), Qt::QueuedConnection);
  connect(this, SIGNAL(signalConnectRtk()), this, SLOT(slotConnectRtk()),
          Qt::QueuedConnection);
  connect(this, SIGNAL(signalDisconnectRtk()), this, SLOT(slotDisconnectRtk()),
          Qt::QueuedConnection);
  connect(this, SIGNAL(signalRequestRtk(int, QString)), this,
          SLOT(slotRequestRtk(int, QString)), Qt::QueuedConnection);
  thread->start();
}
...
void Ntrip::loadCasterList() { emit signalLoadCasterList(); }
...
void Ntrip::slotLoadCasterList() {
  ...
  if (flag) {
    ...
    qWarning() << "Cannot connect to" << addr
               << port; //(QString("Cannot connect to
                        //%1:%2").arg(addr).arg(port).toUtf8().data());
  }
}

void Ntrip::connectRtk() { emit signalConnectRtk(); }

void Ntrip::disconnectRtk() { emit signalDisconnectRtk(); }
...
void Ntrip::slotRequestRtk(int casterId, QString GPGGA) {
  ...
  QString message = QString("GET %1/%2 HTTP/1.1\r\n"
                            ...
                            "%5\r\n")
                        .arg("")
                        .arg(casterlist[casterId].mountpoint)
                        .arg("CMU-CORAL")
                        .arg(base64)
                        .arg(GPGGA);
  this->write(message.toUtf8());
